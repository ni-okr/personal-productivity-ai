...
export async function mockCancelSubscription(subscriptionId: string): Promise<{
    ...
    console.log('Available subscriptions:', mockSubscriptions.map(s => s.id))
...
// Функция для обновления тестовой подписки
export function updateMockSubscription(subscriptionId: string, updates: Partial<Subscription>): void {
    console.log('🧪 MOCK РЕЖИМ: Обновление тестовой подписки')
    const subscriptionIndex = mockSubscriptions.findIndex(sub => sub.id === subscriptionId)
    if (subscriptionIndex !== -1) {
        mockSubscriptions[subscriptionIndex] = {
            ...mockSubscriptions[subscriptionIndex],
